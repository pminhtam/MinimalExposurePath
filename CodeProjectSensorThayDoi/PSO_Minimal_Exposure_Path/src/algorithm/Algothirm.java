...
public abstract class Algothirm {
	...
	public void readData(String filename) throws Exception {
		...
		//Sensor với góc và bán kính thay đổi theo mỗi sensor nên cái này không có
//		Sensor.r = Double.parseDouble(s1[2]); // bán kính
//		Sensor.r = 10.0; // bán kínhs

//		Sensor.angle = Double.parseDouble(s1[3]); // Góc

		
		int W = Integer.parseInt(s1[2]);// chiều dài 
		int H = Integer.parseInt(s1[3]);// chiều rộng
		...
		for (int i = 0; i < nse; i++) {
			...
			double Viangle = Double.parseDouble(t[2]); //hướng cảm biến
			double r = Double.parseDouble(t[3]); 	//bán kính
			double angle = Double.parseDouble(t[4]);//Góc
			list.add(new Sensor(p, Viangle,r,angle));
		}
	...
	public void draw(String filename) {
		...
		for (int i = 0; i < ob.sensors.size(); i++) {
			...
			// Dao nguoc toa do. Toa do ve = p(x, h-y)
			g.fillArc((int) (s.p.x - s.r)*scale, (int) (s.p.y - s.r)*scale, (int) (2 * s.r)*scale, (int) (2 * s.r)*scale,
					(int) ((-s.Viangle - s.angle) * 180 / Math.PI), (int) (s.angle * 360 / Math.PI));
		}
